...
impl Document {
    ...
    pub fn to_mt940(&self) -> Result<Mt940Wrapper, ParsingError> {
        ...
        let field_64 = self.find_balance_by_type("CLAV")
            .map(|bal| {
                Ok(Field64 {
                    value_date: self.parse_date(&bal.dt.dt)
                        .unwrap_or_else(|_| NaiveDate::from_ymd_opt(2023, 12, 31)
                            .ok_or_else(|| ParsingError::ParseDateError("Invalid fallback date".to_string()))?),
                    debit_credit_mark: self.convert_debit_credit(&bal.cdt_dbt_ind)
                        .unwrap_or_else(|_| "C".to_string()),
                    currency: bal.amt.ccy.clone(),
                    amount: self.parse_amount(&bal.amt.value)
